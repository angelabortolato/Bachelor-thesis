...
int main() {
    ...
    std::ofstream fJstep[n_trials/500];
    for (int i = 0; i < n_trials/500; i++)
    {
        fJstep[i].open("Jstep"+std::to_string(i)+".txt");
    }
    int step=0;
    ...
    std::ifstream ifchweights;
    ifchweights.open("index_changeweights.txt");

    int ind_ch_weights[10];

    for (int w = 0; w < 10; w++)
    {
        ifchweights>>ind_ch_weights[w];
    }

    std::ofstream ofchweights;
    ofchweights.open("step_changeweights.txt");
    ofchweights <<"# 10 most changing weights are of index: ";
    for (int w = 0; w < 10; w++)
    {
        ofchweights <<floor(ind_ch_weights[w]/200) <<"," <<ind_ch_weights[w]-floor(ind_ch_weights[w]/200)*200 <<"\t" ;
    }
    ofchweights <<std::endl;
    ...
    for (int ntime = 0; ntime < tottime; ntime++) {
		...
		*/
        ...
        if (trialtime == T - 1) {
            ...
            if (curr_trial%20==0) {         //ogni 20 trial stampo i pesi che cambiano di più
        	    ofchweights << curr_trial <<"\t";
                for (int w = 0; w < 10; w++)
                {
                    ofchweights <<J[ind_ch_weights[w]] <<"\t";
                }
            ...
            if (curr_trial%500==0) {         //ogni 500 trial stampo la matrice J
                for (int i = 0; i < N; i++) {
    	            for (int j = 0; j < N; j++) {
        	            fJstep[step] << J[N*i+j] << "\t";
                    }
     	            fJstep[step] << std::endl;
                }
                step++;
            }
